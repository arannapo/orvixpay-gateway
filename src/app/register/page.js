  ...
  const handleSubmit = async (e) => {
    ...
    if (step === 'credentials') {
      if (!formData.businessName.trim()) {
        toast.error(language === 'es' ? 'El nombre de la empresa es requerido' : 'Business name is required');
        return;
      }
      ...
      try {
        ...
        if (data.success && data.emailOtpRequired) {
          ...
          setOtpCode('');
          setResendCooldown(60);
          toast.success(language === 'es' ? 'Se ha enviado un código de verificación a su correo.' : 'A verification code has been sent to your email.');
        } else {
          toast.error(data.error || (language === 'es' ? 'Error al registrarse' : 'Registration failed'));
        }
      } catch (err) {
      ...
      try {
        ...
        if (data.success) {
          toast.success(language === 'es' ? '¡Correo verificado! Redirigiendo al panel...' : 'Email verified! Redirecting to dashboard...');
          await checkAuth();
          router.push('/dashboard');
        } else {
          toast.error(data.error || (language === 'es' ? 'Error en la verificación' : 'Verification failed'));
        }
      } catch (err) {
        toast.error(language === 'es' ? 'Ocurrió un error durante la verificación.' : 'An error occurred during verification.');
      } finally {
        setLoading(false);
      }
    }
        ...
        <div className="max-w-[360px] w-full relative z-10">
          ...
          <div className="text-center mb-8">
            ...
            <h2 className="text-2xl font-extrabold text-slate-900 tracking-tight">
              {step === 'credentials' ? t("authRegisterTitle") : t("authEmailOtpTitle")}
            </h2>
            <p className="text-sm text-slate-500 mt-2 font-medium">
              {step === 'credentials' ? t("authRegisterSub") : `${t("authEmailOtpSub")} ${maskedEmail}`}
            </p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            {step === 'credentials' && (
              <>
                <div>
                  <label className="block text-xs font-bold text-slate-500 uppercase tracking-widest mb-2">{t("authBusinessName")}</label>
                  <input
                    type="text" autoComplete="off" required
                    placeholder="Acme Corp"
                    className="w-full px-5 py-3.5 bg-slate-55 border border-slate-205 rounded-xl focus:bg-white focus:ring-2 focus:ring-purple-100 focus:border-purple-250 transition-all outline-none font-medium text-slate-900 placeholder:text-slate-400"
                ...
                <div>
                  <label className="block text-xs font-bold text-slate-500 uppercase tracking-widest mb-2">{t("authPass")}</label>
                  <div className="relative">
                    <input
                      type={showPassword ? "text" : "password"} autoComplete="off" required
                      placeholder="••••••••"
                      className="w-full px-5 py-3.5 bg-slate-55 border border-slate-205 rounded-xl focus:bg-white focus:ring-2 focus:ring-purple-100 focus:border-purple-255 pr-12 transition-all outline-none font-medium text-slate-900 placeholder:text-slate-400"
            ...
            {/* Terms policy info (credentials step only) */}
            {step === 'credentials' && (
              <p className="text-slate-450 text-[11px] leading-relaxed font-semibold">
                {language === 'es' 
                  ? 'Al registrarse, acepta los Términos de Servicio y la Política de Privacidad de OrvixPay.' 
                  : 'By registering, you agree to the OrvixPay Terms of Service and Privacy Policy.'}
              </p>
            )}
            ...
            <button type="submit" disabled={loading}
              ...
              {loading ? (
                <><span className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin"></span>
                  {step === 'credentials' ? t("authSignUpLoading") : t("authVerifying")}</>
              ) : step === 'credentials' ? t("authSignUpBtn") : t("authVerifyBtn")}
            </button>
          ...
          <div className="mt-8 text-center text-sm font-medium text-slate-500">
            {t("authAlreadyHaveAccount")} <Link href="/login" className="text-slate-900 hover:text-purple-600 transition-colors font-bold">{t("authLoginHere")}</Link>
          </div>
