...
export default function NewInvoicePage() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!merchantLogo) {
      toast.error(language === 'es' ? 'Por favor suba el logotipo de su empresa en Configuración antes de crear facturas.' : 'Please upload your Business Logo in Settings before creating invoices');
      return;
    }

    if (!formData.amount) {
      toast.error(language === 'es' ? 'El monto es requerido' : 'Amount is required');
      return;
    }
    if (parseFloat(formData.amount) <= 0) {
      toast.error(language === 'es' ? 'El monto debe ser mayor a cero' : 'Amount must be greater than zero');
      return;
    }
    if (!formData.orderId.trim()) {
      toast.error(language === 'es' ? 'El ID de orden es requerido' : 'Order ID is required');
      return;
    }
    if (!formData.customerEmail.trim()) {
      toast.error(language === 'es' ? 'El correo del cliente es requerido' : 'Customer email is required');
      return;
    }
    ...
    try {
      ...
      if (data.success) {
        toast.success(language === 'es' ? '¡Factura creada con éxito!' : 'Invoice created successfully!');
        window.location.href = data.paymentUrl;
      } else {
        toast.error(data.error || (language === 'es' ? 'Error al crear la factura' : 'Failed to create invoice'));
        setLoading(false);
      }
    } catch (error) {
      console.error(error);
      toast.error(language === 'es' ? 'Error de conexión con la API' : 'Error connecting to API');
      setLoading(false);
    }
  };
    ...
    <div className="max-w-5xl mx-auto p-6 md:p-8">
      <div className="flex flex-col lg:flex-row gap-6 items-stretch">
        ...
        <div className="flex-1 bg-white rounded-2xl border border-slate-200/80 shadow-sm flex flex-col w-full">
          <div className="relative overflow-hidden px-6 py-6 border-b border-slate-100 flex flex-col gap-1.5 bg-white rounded-t-2xl">
            ...
            <div className="relative z-10 flex flex-col gap-1">
              <h2 className="text-xl font-extrabold text-slate-900 tracking-tight leading-none">{language === 'es' ? 'Crear Factura' : 'Create Invoice'}</h2>
              <p className="text-slate-500 font-medium text-xs">{language === 'es' ? 'Generar una solicitud de pago manual.' : 'Generate a manual payment request.'}</p>
            </div>
          </div>
          <div className="p-6">
            {!merchantLogo && (
              <div className="mb-4 p-3.5 bg-rose-50 border border-rose-100 rounded-xl text-rose-700 text-xs font-semibold flex items-center gap-2">
                ...
                <span>{language === 'es' ? 'Debe subir el logotipo de su empresa en la Configuración antes de poder generar facturas.' : 'You must upload your Business Logo in Settings before you can generate invoices.'}</span>
              </div>
            )}
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className="block text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-1.5">{language === 'es' ? 'Monto (USD)' : 'Amount (USD)'}</label>
                  <input 
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-1.5">{language === 'es' ? 'Descripción' : 'Description'}</label>
                  <input 
                    type="text" 
                    value={formData.description} 
                    onChange={(e) => setFormData({...formData, description: e.target.value})} 
                    placeholder={language === 'es' ? 'Descripción de bienes o servicios' : 'Services or goods description'}
                    className="w-full px-4 py-2.5 border border-slate-200 rounded-xl focus:bg-white focus:ring-2 focus:ring-purple-100 focus:border-purple-250 transition-all outline-none font-medium text-sm text-slate-900 placeholder:text-slate-400 bg-slate-50/50 hover:bg-slate-50" 
                  />
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className="block text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-1.5">{language === 'es' ? 'Nombre del Cliente (Opcional)' : 'Customer Name (Optional)'}</label>
                  <input 
                    type="text" 
                ...
                <div>
                  <label className="block text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-1.5">{language === 'es' ? 'Correo del Cliente' : 'Customer Email'}</label>
                  <input 
                    type="email" 
              ...
              <div className="pt-4 flex gap-3">
                ...
                >
                  {loading ? (
                    <><span className="w-3.5 h-3.5 border-2 border-white border-t-transparent rounded-full animate-spin"></span> {language === 'es' ? 'Creando...' : 'Creating...'}</>
                  ) : (
                    <>
                      <DollarSign size={14} /> {language === 'es' ? 'Crear Factura' : 'Create Invoice'}
                    </>
                  )}
                </button>
                <button 
        ...
        <div className="w-full lg:w-[350px] shrink-0 flex flex-col">
          <h3 className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-3 px-1">{language === 'es' ? 'Vista Previa en Vivo' : 'Live Preview'}</h3>
            ...
            <div className="relative z-10 flex-1 flex flex-col">
              ...
              <div className="flex justify-between items-start mb-6 border-b border-slate-200/50 pb-4">
                <div>
                  ...
                  <p className="text-slate-400 text-[10px] font-semibold uppercase tracking-wider mb-0.5">{language === 'es' ? 'Facturado A' : 'Billed To'}</p>
                  <p className="text-slate-800 font-bold text-sm leading-tight">{formData.customerName || (language === 'es' ? 'Nombre Cliente' : 'Customer Name')}</p>
                  <p className="text-slate-500 text-[11px] font-mono leading-none mt-1">{formData.customerEmail || 'customer@email.com'}</p>
                </div>
                <div className="text-right">
                  <p className="text-slate-400 text-[9px] font-bold uppercase tracking-widest mb-0.5">{language === 'es' ? 'Factura' : 'Invoice'}</p>
              ...
              <div className="space-y-4">
                ...
                <div className="bg-white/60 rounded-xl p-4 border border-white backdrop-blur-sm shadow-sm flex justify-between items-center">
                  <div>
                    <p className="text-slate-500 text-xs font-semibold mb-1">{language === 'es' ? 'Monto Vencido' : 'Amount Due'}</p>
                ...
                <div className="grid grid-cols-2 gap-4 px-0.5">
                  ...
                  <div>
                    <p className="text-slate-400 text-[10px] font-semibold uppercase tracking-wider mb-0.5">{language === 'es' ? 'Estado' : 'Status'}</p>
                ...
                <div className="px-0.5 pt-2">
                  <p className="text-slate-400 text-[10px] font-semibold uppercase tracking-wider mb-1">{language === 'es' ? 'Descripción' : 'Description'}</p>
                  <p className="text-slate-700 text-xs leading-relaxed">
                    {formData.description || (language === 'es' ? 'Sin descripción proporcionada.' : 'No description provided.')}
                  </p>
                </div>
              </div>
              ...
              <div className="mt-auto pt-8">
                <button disabled className="w-full py-2.5 bg-slate-950 text-white rounded-xl font-bold text-xs shadow-sm opacity-50 cursor-not-allowed">
                  {language === 'es' ? 'Pagar Factura' : 'Pay Invoice'}
                </button>
              </div>
            </div>
          </div>
